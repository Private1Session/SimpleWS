...
class entity {
	constructor(team,stats) {
		...
		this.deployWait = RandomInt(0,200)
		this.isMoving = false;
...
class bomber extends entity {
	...
	onLeaveScreen(){
		...
		this.health = Math.min(maxHealth,this.health + Math.ceil(maxHealth/2.5));
		this.custom.range = Math.floor(-this.y/4)
	}

	attack() {
		const lifetime = Math.floor(Math.abs(this.y) / 10);
		let moveX = this.custom.mobility;
